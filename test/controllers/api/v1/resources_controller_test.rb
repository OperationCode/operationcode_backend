...
class Api::V1::ResourcesControllerTest < ActionDispatch::IntegrationTest
  ...
  test ":index endpoint returns JSON list of Resources" do
    get api_v1_resources_url, headers: @headers, as: :json

    assert_equal response.status, 200
    [@books.name, @videos.name].each do |resource|
      assert_equal true, response.parsed_body.any? { |response| response['name'] == resource }
    end
  end
  ...
  test ":show endpoint returns JSON representation of the requested Resource" do
    get api_v1_resource_url(@books.id), headers: @headers, as: :json
