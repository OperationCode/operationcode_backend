...
class UserTest < ActiveSupport::TestCase
  ...
  test '.community_leaders_within returns leaders within the radius of a location' do
    tom = create :user, latitude: 1, longitude: 1
    sam = create :user, latitude: 1.0001, longitude: 1.0001
    sam.tag_list.add("community-leader")
    tom.tag_list.add("community-leader")

    tom.stubs(:geocode)
    sam.stubs(:geocode)
    
    results = User.community_leaders_within(1,1,10)
